...
All code referenced from the mod description is labeled from their originating mod with function purpouses
...
config.complex_jokers.last_option = {}
config.tarot_texture.last_option = {}
...
furry_mod.config_tab = function() -- Config Tab
    return {n = G.UIT.ROOT, config = {align = "m", r = 0.1, padding = 0.1, colour = G.C.BLACK, minw = 8, minh = 6}, nodes = {
		...
		create_option_cycle {
        	label = localize("fur_tarottexture"),
        	options = { localize("fur_default"), localize("fur_scruffybrush") },
        	current_option = config.tarot_texture.current_option,
        	ref_table = config,
        	ref_value = "tarot_texture",
        	opt_callback = 'fur_togglecycle',
        	w = 5.5
        },
    }}
...
furry_mod.optional_features = function() -- Mod features
	return {
...
Game.main_menu = function(change_context)
	...
	for _, v in pairs(G.P_CENTERS) do
		if Cryptid then
			if v.rarity == "fur_rarityfurrynonmythic" and v.key == "j_fur_sparkles" then
				table.insert(possible, v.key)
			end
		elseif v.rarity == "fur_rarityfurrynonmythic" or v.rarity == "fur_rarityfurrymythic" then
			table.insert(possible, v.key)
		end
	end
...
G.C.FUR_BUGSQUASHER_A = { 0, 0, 0, 0 }
G.C.FUR_BUGSQUASHER_B = { 0, 0, 0, 0 }
G.C.FUR_DEVELOPER = { 0, 0, 0, 0 }
G.C.FUR_VGEN = { 0, 0, 0, 0 }
G.C.FUR_DISCORD = { 0, 0, 0, 0 }
G.C.FUR_RAFFLE = { 0, 0, 0, 0 }
G.C.FUR_HALLOWEEN = { 0, 0, 0, 0 }
G.C.FUR_XMAS = { 0, 0, 0, 0 }
...
furry_mod.C = {
	...
	BUGSQUASHER_A = { HEX("141035"), HEX("FFFFFF") },
	BUGSQUASHER_B = { HEX("FFFFFF"), HEX("141035") },
	DEVELOPER = { HEX("1A98AC"), HEX("DFA84B") },
	RAFFLE = { HEX("3DFFF4"), HEX("8F00FF") },
	HALLOWEEN = { HEX("FFA500"), HEX("000000") },
	XMAS = { HEX("FF0000"), HEX("00FF00") },
...
local fur_badge = {
	...
	literally_tenna = {
		col = G.C.FUR_DISCORD,
        text = {
            'Idea: literally_tenna (MR.L)',
            'Discord Community Member',
        }
	},
	bugsquasher = {
		col = G.C.FUR_BUGSQUASHER_A,
		tcol = G.C.FUR_BUGSQUASHER_B,
        text = {
            'Bug Squasher',
        }
	},
...
G.FUNCS.scruffybrush = function(e)
    love.system.openURL("https://scruffybrush.ca/")
end

function set_sticker_modifiers(card) -- For stake stickers
...
-- Help from TheOneGoofAli (Hooking for challs and other stuffs)
local pollediref = poll_edition -- Maxing Challenges edition hook
function poll_edition(_key, _mod, _no_neg, _guaranteed)
	...
	if _guaranteed then
        ...
        elseif edition_poll > 1 - 0.006*25 then
			if G.GAME.challenge == "c_fur_challengechipsmaxing" then
				return {foil = true}
			else
        ...
        elseif edition_poll > 1 - 0.04*25 then
			if G.GAME.challenge == "c_fur_challengemultmaxing" then
				return {holo = true}
			else
            	return {foil = true}
			end
...
---------------------------------------------TABLES------------------------------------------------------------
SMODS.Rarity { -- Non-Mythics
    key = 'rarityfurrynonmythic',
    badge_colour = G.C.DARK_EDITION,
	default_weight = 0.1,
	pools = { ["Joker"] = true },
	get_weight = function(self, weight, object_type)
		if next(SMODS.find_card("v_fur_furrymeetup")) then
			return 0.1
		else
			return 0
		end
...
SMODS.Rarity { -- Mythics
    key = 'rarityfurrymythic',
    badge_colour = G.C.DARK_EDITION,
	default_weight = 0.04,
	pools = { ["Joker"] = true },
	get_weight = function(self, weight, object_type)
		if next(SMODS.find_card("v_fur_furryconvention")) then
			return 0.04
		else
			return 0
		end
...
SMODS.Rarity { -- Corrupted
    key = 'rarityfurrycorrupt',
    badge_colour = G.C.DARK_EDITION,
}
...
SMODS.ConsumableType { -- Joker Equipment
    key = "jokerconsumables",
    loc_txt = {
        name = "Joker Consumables",
        collection = "Joker Consumables",
        undiscovered = {
            name = "Not Discovered",
            text = {
                "Purchase or use",
    ...
    primary_colour = HEX("B41B1D"),
    secondary_colour = HEX("6EB492"),
    collection_rows = { 6, 2 },
    no_collection = true,
}
---------------------------------------------------------------------------------------------------------------