    ...
    calc_function = function(card)
        ...
        for _, scoring_card in pairs(G.hand.highlighted) do
            if scoring_card.config.center.key == "m_fur_ghostcard" then
                if scoring_card:is_suit("fur_stars") or scoring_card:is_suit("six_Stars") then
...
jd_def["j_fur_diablo2324"] = { -- Diablo 2324
    ...
    calc_function = function(card)
        ...
        card.joker_display_values.localized_text = localize("k_fur_rarityfurrynonmythic")
    end
}
...
if Talisman then -- Sourstone :3
    jd_def["j_fur_talismansourstone3"] = {
        ...
        calc_function = function(card)
            ...
            elseif G.jokers.cards[1] == card and G.jokers.cards[#G.jokers.cards] == card then
                money = 0
                emult = card.ability.extra.emult
            else
                money = 0
...
jd_def["j_fur_parrotdash"] = { -- Parrot Dash
    text = {
        { text = "+", colour = G.C.MULT },
        { ref_table = "card.joker_display_values", ref_value = "total", colour = G.C.MULT },
    },

    calc_function = function(card)
        if next(SMODS.find_mod("Multiplayer")) then
            if MP.GAME.lives >= 1 then
                card.joker_display_values.total = card.ability.extra.mult + (card.ability.fakeante * card.ability.fakerounds)
            else
                card.joker_display_values.total = card.ability.extra.mult + (G.GAME.round_resets.ante * G.GAME.round)
            end
        else
            card.joker_display_values.total = card.ability.extra.mult + (G.GAME.round_resets.ante * G.GAME.round)
        end
...
jd_def["j_fur_ych"] = { -- YCH
    reminder_text = {
        { text = "(" },
        { ref_table = "card.ability.extra", ref_value = "roundsplayed", colour = G.C.FILTER },
        { text = "/" },
        { ref_table = "card.ability.extra", ref_value = "roundstotal", colour = G.C.UI.TEXT_INACTIVE },
        { text = ")" }
    },
}

jd_def["j_fur_gale"] = { -- Gale
    text = {
        { text = "+", colour = G.C.CHIPS },
        { ref_table = "card.joker_display_values", ref_value = "chipstotal", colour = G.C.CHIPS },
        { text = " " },
        { text = "+", colour = G.C.MULT },
        { ref_table = "card.joker_display_values", ref_value = "multtotal", colour = G.C.MULT },
        { text = " " },
        {
            border_nodes = {
                { text = "X" },
    ...
    calc_function = function(card)
        local decksnowflakecardcount = 0
        local playedsnowflakecardcount = 0
        local xmulttotal = 1
        for _, playing_card in ipairs(G.playing_cards) do
            if SMODS.has_enhancement(playing_card, 'm_fur_snowflakecard') then 
                decksnowflakecardcount = decksnowflakecardcount + 1 
            end
        end
        ...
        if text ~= 'Unknown' then
            for _, scoring_card in pairs(scoring_hand) do
                if scoring_card.config.center.key == "m_fur_snowflakecard" then
                    playedsnowflakecardcount = playedsnowflakecardcount + 1
                end
            end
            
            if playedsnowflakecardcount > 0 and playedsnowflakecardcount == #scoring_hand then
                xmulttotal = card.ability.extra.xmult
            end
        end

        card.joker_display_values.xmult = xmulttotal
        card.joker_display_values.chipstotal = card.ability.extra.chips + (card.ability.extra.chipsgain * decksnowflakecardcount)
        card.joker_display_values.multtotal = card.ability.extra.mult + (card.ability.extra.multgain * decksnowflakecardcount)
    end
}

jd_def["j_fur_potmario"] = { -- PotMario
    text = {
        { text = "+", colour = G.C.CHIPS },
        { ref_table = "card.joker_display_values", ref_value = "chipstotal", colour = G.C.CHIPS },
        { text = " " },
        ...
        { text = " " },
        { text = "+$", colour = G.C.MONEY },
        { ref_table = "card.joker_display_values", ref_value = "moneytotal", colour = G.C.MONEY },
    },
    
    calc_function = function(card)
        local furryjokercount = 0
        local moneyamount = 0
        for i = 1, #G.jokers.cards do
	    	if G.jokers.cards[i].config.center.rarity == "fur_rarityfurrynonmythic" or G.jokers.cards[i].config.center.rarity == "fur_rarityfurrymythic" or G.jokers.cards[i].config.center.rarity == "fur_rarityfurrycorrupt" then
                furryjokercount = furryjokercount + 1
            end
        end
        if G.jokers.cards[1] == card and G.jokers.cards[#G.jokers.cards] == card then
            moneyamount = card.ability.extra.money
        end
        

        card.joker_display_values.chipstotal = card.ability.extra.chips + (card.ability.extra.chipsgain * furryjokercount)
        card.joker_display_values.xmult = card.ability.extra.xmult + (card.ability.extra.xmultgain * furryjokercount)
        card.joker_display_values.moneytotal = moneyamount
    end
}

--[[if Talisman then -- AberrantReptile (old effect)
    if config.jokerdisplay_scaling then
        jd_def["j_fur_talismanaberrantreptile"] = { 
            text = {
                {
                    border_nodes = {
                        { text = "+^" },
            ...
            calc_function = function(card)
                card.joker_display_values.localized_text = localize("fur_jdcardcreated")
            end
        }
    else
        jd_def["j_fur_talismanaberrantreptile"] = { 
...
jd_def["j_fur_aberrantreptile"] = { -- AberrantReptile
    text = {
        { text = "+", scale = 0.35 },
        { ref_table = "card.joker_display_values", ref_value = "blueseals" },
        { text = " ", scale = 0.35 },
        { ref_table = "card.joker_display_values", ref_value = "localized_text", colour = G.C.DARK_EDITION },
    },
    reminder_text = {
        { text = "(" },
        { ref_table = "card.joker_display_values", ref_value = "localized_text_two", colour = G.C.ORANGE },
        { text = ")" },
    },
    ...
    calc_function = function(card)
        ...
        if G.play then
            ...
            for _, scoring_card in pairs(G.hand.highlighted) do
                if scoring_card.config.center.key == "m_fur_ghostcard" then
                    if scoring_card.seal == "Blue" then
                        ghostcards = ghostcards + 1
                    end
                end
            end
        ...
        card.joker_display_values.blueseals = count + ghostcards
        card.joker_display_values.localized_text = localize('k_planet')
        card.joker_display_values.localized_text_two = localize("blue_seal", "labels")
    end
}

if Talisman then -- iridia (Corrupted)
    jd_def["j_fur_talismancorruptiridia"] = { 
        text = {
            {
                border_nodes = {
                    { text = "^" },
        ...
        reminder_text = {
            ...
            { ref_table = "card.joker_display_values", ref_value = "localized_text", colour = G.C.SUITS.Clubs },
            { text = "," },
            { ref_table = "card.joker_display_values", ref_value = "localized_text2", colour = G.C.SUITS.Diamonds },
            { text = ")" },
        },
...
jd_def["j_caino"] = { -- Canio Ownership JD fix
    text = {
        {
            border_nodes = {
                { text = "X" },
