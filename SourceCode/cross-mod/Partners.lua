    ...
    calculate = function(self, card, context)
        ...
        if context.individual and context.cardarea == G.play then
            ...
            for i = 1, #context.scoring_hand do
                if context.scoring_hand[i]:is_suit("fur_stars") then
        ...
        if link_level ~= 1 then
            if context.hand_drawn then
                for i = 1, #G.playing_cards do
                    if not G.playing_cards[i]:is_suit("fur_stars") then
...
Partner_API.Partner { -- Parrot Dash
    ...
    pos = { x = 1, y = 4 },
    atlas = "FurryPartners",
    config = { roundsplayed = 0, fakerounds = 0, fakeante = 1, extra = { mult = 2 }},
    ...
    calculate = function(self, card, context)
        ...
        if link_level ~= 1 then
            if context.setting_blind or context.skip_blind then
                ...
                if card.ability.roundsplayed == 3 then
                    card.ability.roundsplayed = 0
                    card.ability.fakeante = card.ability.fakeante + 1
                else
                    card.ability.roundsplayed = card.ability.roundsplayed + 1
                end
            end
