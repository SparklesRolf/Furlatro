Partner_API.Partner { -- AberrantReptile
    key = "aberrantreptilepartner",
    name = "AberrantReptile (Partner)",
    unlocked = true,
    discovered = true,
    pos = { x = 4, y = 4 },
    atlas = "FurryPartners",
    config = { bluesealtriggersremaining = 2, extra = { retriggers = 1 }},
    link_config = { j_fur_aberrantreptile = 1 },
        ...
        if link_level ~= 1 then
            if context.repetition and context.cardarea == G.play then
                ...
                if playingcard.seal == "Blue" then
                    if card.ability.bluesealtriggersremaining <= 1 then
                        card.ability.bluesealtriggersremaining = 2
                        local _planet = nil
                        ...
                        card.ability.bluesealtriggersremaining = card.ability.bluesealtriggersremaining - 1
                    end
                end
            end
        end
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return { key = key, vars = { card.ability.bluesealtriggersremaining, card.ability.extra.retriggers, localize('negative', 'labels'), localize('k_planet') }}
    end,

    check_for_unlock = function(self, args)
        for _, v in pairs(G.P_CENTER_POOLS["Joker"]) do
            if v.key == "j_fur_aberrantreptile" then
...
Partner_API.Partner { -- Danny
    ...
    calculate = function(self, card, context)
        ...
        if link_level ~= 1 then
            if context.hand_drawn or context.before or context.end_of_round or context.open_booster or context.selling_card then
                for i = 1, #G.playing_cards do
	                if not (G.playing_cards[i]:is_suit("fur_stars") or G.playing_cards[i]:is_suit("six_Stars")) then
	                	SMODS.debuff_card(G.playing_cards[i], true, "fur_danny")
                    else
                        SMODS.debuff_card(G.playing_cards[i], false, "fur_danny")
                    end
	            end
            end

            if context.using_consumeable then
                if (context.consumeable.config.center.key == 'c_fur_fallingstar' or context.consumeable.config.center.key == 'c_six_star_q') then
	                for i = 1, #G.hand.highlighted do
                        SMODS.debuff_card(G.hand.highlighted[i], false, "fur_danny")
                    end
                elseif (context.consumeable.config.center.key == 'c_sun' or context.consumeable.config.center.key == 'c_moon' or context.consumeable.config.center.key == 'c_world' or context.consumeable.config.center.key == 'c_star' or context.consumeable.config.center.key == 'c_six_moon_q') then
                    for i = 1, #G.hand.highlighted do
                        SMODS.debuff_card(G.hand.highlighted[i], true, "fur_danny")
                    end
                end
            end
...
Partner_API.Partner { -- Diablo2324
...
Partner_API.Partner { -- Gale
    ...
    pos = { x = 5, y = 4 },
    atlas = "FurryPartners",
    config = { extra = { chips = 0, mult = 0, chipsgain = 50, multgain = 5, xmult = 1.5 }},
    link_config = { j_fur_gale = 1 },
    ...
    calculate = function(self, card, context)
        ...
        if link_level ~= 1 then
            if context.before then
                local first_card = {}
                local convertedcards = false
                for i = 1, #context.full_hand do
                    if context.full_hand[i].debuff or SMODS.has_enhancement(context.full_hand[i], 'm_stone') then
                        first_card = context.full_hand[i]
                        break
                    end
                end

                if first_card then
                    first_card:set_ability(G.P_CENTERS['m_fur_snowflakecard']);
                    SMODS.debuff_card(first_card, false, "fur_danny")
                    convertedcards = true
                ...
                if convertedcards then
                    return {
                        message = localize('fur_snowflakecardtrigger'),
                        colour = G.C.FILTER
                    }
                end
            end
            ...
            local snowflakecardcount = 0
            for _, playing_card in ipairs(G.playing_cards) do
                if SMODS.has_enhancement(playing_card, 'm_fur_snowflakecard') then 
                    snowflakecardcount = snowflakecardcount + 1 
                end
            end

            return {
                chips = card.ability.extra.chips + (card.ability.extra.chipsgain * snowflakecardcount),
                mult = card.ability.extra.mult + (card.ability.extra.multgain * snowflakecardcount),
            }
            end
            ...
            for k, v in ipairs(context.scoring_hand) do
				if v.config.center.key == "m_fur_snowflakecard" then
                    snowflakecardcount = snowflakecardcount + 1
                end
			end

            if snowflakecardcount == #context.full_hand then
                return {
                    xmult = card.ability.extra.xmult
                }
            end
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return { key = key, vars = { card.ability.extra.chipsgain, card.ability.extra.multgain, card.ability.extra.chips + (card.ability.extra.chipsgain * snowflakecardcount), card.ability.extra.mult + (card.ability.extra.multgain * snowflakecardcount), card.ability.extra.xmult}}     
    end,

    check_for_unlock = function(self, args)
        for _, v in pairs(G.P_CENTER_POOLS["Joker"]) do
            if v.key == "j_fur_gale" then
                if get_joker_win_sticker(v, true) >= 8 then
                    return true
                end
                break
...
Partner_API.Partner { -- IgnarilAssacire
    key = "ignarilassacirepartner",
    name = "Ignaril Assacire (Partner)",
    unlocked = true,
    discovered = true,
    pos = { x = 0, y = 5 },
    atlas = "FurryPartners",
    config = { retriggerisactive = false, triggersremaining = 2, retriggercard = nil, extra = { retriggers = 1 }},
    link_config = { j_fur_ignarilassacire = 1 },
    ...
    calculate = function(self, card, context)
        ...
        if link_level ~= 1 then
            if context.post_trigger and (context.other_card.config.center.rarity == "fur_rarityfurrynonmythic" or context.other_card.config.center.rarity == "fur_rarityfurrymythic" or context.other_card.config.center.rarity == "fur_rarityfurrycorrupt") then
                if card.ability.triggersremaining == 1 then
                    card.ability.triggersremaining = 2
                    card.ability.retriggerisactive = true
                    card.ability.retriggercard = context.other_card.label
                else
                    card.ability.triggersremaining = card.ability.triggersremaining - 1
                end
            end

            if card.ability.retriggerisactive then
                for i = 1, #G.jokers.cards do 
                    if G.jokers.cards[i].label == card.ability.retriggercard then
                        local ret = SMODS.blueprint_effect(card, G.jokers.cards[i], context.other_context)
                        if ret then
                            ret.colour = G.C.FILTER
                        end
                        card.ability.retriggerisactive = false
                        return ret
                    end
                end
            end
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return { key = key, vars = { card.ability.triggersremaining, card.ability.extra.retriggers }}
    end,

    check_for_unlock = function(self, args)
        for _, v in pairs(G.P_CENTER_POOLS["Joker"]) do
            if v.key == "j_fur_ignarilassacire" then
                if get_joker_win_sticker(v, true) >= 8 then
                    return true
                end
                break
...
Partner_API.Partner { -- KalikHusky
    key = "kalikpartner",
    name = "Kalik (Partner)",
    unlocked = false,
    discovered = true,
...
Partner_API.Partner { -- Luposity
    ...
    calculate = function(self, card, context)
        ...
        if context.end_of_round then
            if not card.ability.cookiejarfilled then
...
Partner_API.Partner { -- PotMario
    key = "potmariopartner",
    name = "PotMario (Partner)",
    unlocked = true,
    discovered = true,
    pos = { x = 1, y = 5 },
    atlas = "FurryPartners",
    config = { extra = { chips = 0, xmult = 1, chipsgain = 8, xmultgain = 0.25, money = 5 }},
    link_config = { j_fur_potmario = 1 },
    ...
    calculate = function(self, card, context)
        ...
        if link_level ~= 1 then
            if context.joker_main then
                ...
                for i = 1, #G.jokers.cards do
                    if G.jokers.cards[i].config.center.rarity == "fur_rarityfurrynonmythic" or G.jokers.cards[i].config.center.rarity == "fur_rarityfurrymythic" or G.jokers.cards[i].config.center.rarity == "fur_rarityfurrycorrupt" then
                        furrycards = furrycards + 1
                    end
                end

                return {
                    chips = card.ability.extra.chips + (card.ability.extra.chipsgain * furrycards),
                    xmult = card.ability.extra.xmult + (card.ability.extra.xmultgain * furrycards),
                }
            end

            if context.round_eval and G.GAME.blind.boss and #G.jokers.cards == 1 then
            ...
            elseif context.round_eval and #G.jokers.cards == 1 then
                ease_dollars(card.ability.extra.money)
                return {
                    extra = {
                        card = card,
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return { key = key, vars = { card.ability.extra.chips + (card.ability.extra.chipsgain * furrycards), card.ability.extra.xmult + (card.ability.extra.xmultgain * furrycards), card.ability.extra.chipsgain, card.ability.extra.xmultgain, card.ability.extra.money, localize("k_ante")}}
    end,

    check_for_unlock = function(self, args)
        for _, v in pairs(G.P_CENTER_POOLS["Joker"]) do
            if v.key == "j_fur_potmario" then
                if get_joker_win_sticker(v, true) >= 8 then
                    return true
                end
                break
...
Partner_API.Partner { -- SilverSentinell
    ...
    pos = { x = 5, y = 0 },
    atlas = "FurryPartners",
    config = { spawnstate = "Inactive", extra = { money = 2 }},
...
Partner_API.Partner { -- The_Koneko
    key = "konekopartner",
...
SMODS.Consumable { -- Butter Cookie
    ...
    use = function(self, card, area, copier)
        local increasetargetselection = math.floor(math.random(1, 3))

        if increasetargetselection == 1 then
        ...
        elseif increasetargetselection == 2 then
        ...
        elseif increasetargetselection == 3 then
            ...
            if Talisman then
                if to_number(G.GAME.blind.chips) > 0 then
                    SMODS.draw_cards(1)
                end
...
SMODS.Consumable { -- Double Chocolate Chip Cookie
    ...
    use = function(self, card, area, copier)
        local scoretarget = math.floor(math.random(1, 2))
        ...
        if scoretarget == 1 then
            ...
            if Talisman then
                G.GAME.hands[pokerhandtarget].chips = to_number(G.GAME.hands[pokerhandtarget].chips) * 2
                G.GAME.hands[pokerhandtarget].s_chips = to_number(G.GAME.hands[pokerhandtarget].s_chips) * 2
            else
                G.GAME.hands[pokerhandtarget].chips = G.GAME.hands[pokerhandtarget].chips * 2
                G.GAME.hands[pokerhandtarget].s_chips = G.GAME.hands[pokerhandtarget].s_chips * 2
            end
            delay(1)
        elseif scoretarget == 2 then
            ...
            if Talisman then
                G.GAME.hands[pokerhandtarget].mult = to_number(G.GAME.hands[pokerhandtarget].mult) * 2
                G.GAME.hands[pokerhandtarget].s_mult = to_number(G.GAME.hands[pokerhandtarget].s_mult) * 2
            else
                G.GAME.hands[pokerhandtarget].mult = G.GAME.hands[pokerhandtarget].mult * 2
                G.GAME.hands[pokerhandtarget].s_mult = G.GAME.hands[pokerhandtarget].s_mult * 2
            end
            delay(1)
        end
