    ...
    unlock_condition = function(self, args)
        if args.type == "modify_jokers" then
            if 
                ...
                next(SMODS.find_card("j_fur_lume"))
            then return true end
        end
    end
}
...
SMODS.Achievement { -- Enhanced Spectrum
    ...
    unlock_condition = function(self, args)
        ...
        if args.type == 'hand' then
            ...
            if ghostcards >= 1 and silvercards >= 1 and stinkcards >= 1 and sockcards >= 1 and princecards >= 1 then
                return true
            end
        end
    end
...
SMODS.Achievement { -- You Are Gae
    ...
    unlock_condition = function(self, args)
        if args.type == "modify_jokers" then
            for i = 1, #G.jokers.cards do
				if G.jokers.cards[i].config.center.rarity == "fur_rarityfurry" and (G.jokers.cards[i].edition and G.jokers.cards[i].edition.polychrome) then
					return true
				end
			end
        end
        ...
        if args.type == "modify_jokers" then
            if Talisman then
                if 
                    ...
                    next(SMODS.find_card("j_fur_talismandanny")) and
                    (next(SMODS.find_card("j_fur_skips")) or next(SMODS.find_card("j_fur_multiplayerskips"))) and
                    next(SMODS.find_card("j_fur_ghost")) and
                    next(SMODS.find_card("j_fur_illy")) and
                    next(SMODS.find_card("j_fur_iridia")) and
                    next(SMODS.find_card("j_fur_talismanxavierorjose")) and
                    next(SMODS.find_card("j_fur_talismankris57")) and
                    (next(SMODS.find_card("j_fur_luposity")) or next(SMODS.find_card("j_fur_cryptidluposity"))) and
                    next(SMODS.find_card("j_fur_talismansephirapaws")) and
                    next(SMODS.find_card("j_fur_soks")) and
                    next(SMODS.find_card("j_fur_talismansourstone3"))
                then return true end
            else
                if 
                    next(SMODS.find_card("j_fur_curiousangel")) and
                    ...
                    next(SMODS.find_card("j_fur_illy")) and
                    next(SMODS.find_card("j_fur_iridia")) and
                    next(SMODS.find_card("j_fur_kris57")) and
                    next(SMODS.find_card("j_fur_luposity")) and
                    next(SMODS.find_card("j_fur_soks"))
                then return true end
            end
        end
    end
