...
SMODS.Consumable { -- Floofball
    ...
    use = function(self, card, area, copier)
        play_sound('timpani')
        local card = create_card("furry", G.jokers, nil, nil, nil, nil, nil, 'floofball')
...
SMODS.Consumable { -- ???
        ...
        if #G.jokers.highlighted == 1 then
            for i = 1, #G.jokers.highlighted do
                ...
                elseif G.jokers.highlighted[i].config.center.key == "j_fur_sephirapaws" or G.jokers.highlighted[i].config.center.key == "j_fur_talismansephirapaws" then
                    ...
                    return true
                elseif G.jokers.highlighted[i].config.center.key == "j_fur_iridia" then
                    card.ability.selectedjoker = "j_fur_iridia"
                    return true
                else
                   return false
                end
    ...
    use = function(self, card, area, copier)
        if card.ability.selectedjoker == "j_fur_curiousangel" then
            G.E_MANAGER:add_event(Event({
                func = function()
                    local cardkey = nil
                    if Talisman then
                        cardkey = "j_fur_talismancorruptcuriousangel"
                    else
                        cardkey = "j_fur_corruptcuriousangel"
                    end
                    local corruptcard = SMODS.create_card{set = "Jokers", area = G.jokers, skip_materialize = false, key = cardkey, no_edition = true }
                    set_sticker_modifiers(corruptcard)
        ...
        elseif card.ability.selectedjoker == "j_fur_sparkles" then
            G.E_MANAGER:add_event(Event({
                func = function()
                    local cardkey = nil
                    if Talisman then
                        cardkey = "j_fur_talismancorruptsparkles"
                    else
                        cardkey = "j_fur_corruptsparkles"
                    end
                    local corruptcard = SMODS.create_card{set = "Jokers", area = G.jokers, skip_materialize = false, key = cardkey, no_edition = true }
                    set_sticker_modifiers(corruptcard)
                    corruptcard:start_materialize()
        ...
        elseif card.ability.selectedjoker == "j_fur_danny" then
            G.E_MANAGER:add_event(Event({
                func = function()
                    local cardkey = nil
                    if Talisman then
                        cardkey = "j_fur_talismancorruptdanny"
                    else
                        cardkey = "j_fur_corruptdanny"
                    end
                    local corruptcard = SMODS.create_card{set = "Jokers", area = G.jokers, skip_materialize = false, key = cardkey, no_edition = true }
                    set_sticker_modifiers(corruptcard)
                    corruptcard:start_materialize()
        ...
        elseif card.ability.selectedjoker == "j_fur_sephirapaws" then
            G.E_MANAGER:add_event(Event({
                func = function()
                    local cardkey = nil
                    if Talisman then
                        cardkey = "j_fur_talismancorruptsephirapaws"
                    else
                        cardkey = "j_fur_corruptsephirapaws"
                    end
                    local corruptcard = SMODS.create_card{set = "Jokers", area = G.jokers, skip_materialize = false, key = cardkey, no_edition = true }
                    set_sticker_modifiers(corruptcard)
                    corruptcard:start_materialize()
        ...
        elseif card.ability.selectedjoker == "j_fur_iridia" then
            G.E_MANAGER:add_event(Event({
                func = function()
                    local cardkey = nil
                    if Talisman then
                        cardkey = "j_fur_talismancorruptiridia"
                    else
                        cardkey = "j_fur_corruptiridia"
                    end
                    local corruptcard = SMODS.create_card{set = "Jokers", area = G.jokers, skip_materialize = false, key = cardkey, no_edition = true }
                    set_sticker_modifiers(corruptcard)
                    corruptcard:start_materialize()
...
-- Consumables Texture Pack
SMODS.Consumable:take_ownership("c_fool", {
    scruffybrush = false,

    update = function(self, card, dt)
        local texturepack = config.tarot_texture.current_option
        local consumeableref = card.config.center
        consumeableref.scruffybrush = nil

        if texturepack == 1 then
            consumeableref.atlas = "Tarot"
        elseif texturepack == 2 then
            consumeableref.atlas = "fur_scruffybrushconsumables"
            consumeableref.scruffybrush = true
        end

        if config.tarot_texture.last_option[self.key] ~= config.tarot_texture.current_option then
            config.tarot_texture.last_option[self.key] = config.tarot_texture.current_option
            card:set_sprites(consumeableref)
        end
    end
},true)
