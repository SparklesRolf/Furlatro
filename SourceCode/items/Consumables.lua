...
SMODS.Consumable { -- Nemz's Bow
    ...
    calculate = function(self, card, context)
        if next(SMODS.find_mod("partner")) then
            if G.GAME.selected_partner == "pnr_fur_nemzatapartner" then
                if card.ability.partnersynergizetrigger == "False" or card.ability.extra.ammocap == 3 then
    ...
    can_use = function(self, card)
        if G and G.hand then
            if #G.hand.highlighted ~= 0 and #G.hand.highlighted <= self.config.extra.amount and card.ability.extra.ammo > 0 then
                return true
            end
        end
        return false
    ...
    in_pool = function(self, args)
        ...
        for i = 1, #G.jokers.cards do
            ...
            elseif G.jokers.cards[i].config.center.key == "j_fur_curiousangel" then
                validcards = validcards + 1
            elseif G.jokers.cards[i].config.center.key == "j_fur_danny" or G.jokers.cards[i].config.center.key == "j_fur_talismandanny" then
                validcards = validcards + 1
