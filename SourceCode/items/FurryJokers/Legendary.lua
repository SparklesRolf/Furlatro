    ...
    calculate = function(self, card, context)
        ...
        if next(SMODS.find_mod("partner")) then
            if G.GAME.selected_partner == "pnr_fur_diablo2324partner" then
                if card.ability.partnersynergizetrigger == "False" or card.ability.extra.xmult == 2.5 then
...
SMODS.Joker {
    key = 'foxxlyduskfur',
    config = { partnersynergizetrigger = "False", extra = { xmult = 1, gain = 0.05 }},
    ...
    calculate = function(self, card, context)
        ...
        for i = 1, #G.jokers.cards do
            if next(SMODS.find_card("j_dusk")) then
        ...
        if next(SMODS.find_mod("partner")) then
            if G.GAME.selected_partner == "pnr_fur_foxxlyduskfurpartner" then
                for i = 1, #G.jokers.cards do
                    if next(SMODS.find_card("j_dusk")) and (card.ability.extra.gain == 0.05 or card.ability.extra.gain == 0.1) then
                        card.ability.extra.gain = card.ability.extra.gain * 2
                        break
                    elseif not next(SMODS.find_card("j_dusk")) and card.ability.extra.gain == 0.05 then
                        card.ability.extra.gain = card.ability.extra.gain * 2
                        break
                    elseif not next(SMODS.find_card("j_dusk")) and card.ability.extra.gain ~= 0.05 then
                        card.ability.extra.gain = card.ability.extra.gain / 2
                ...
                for i = 1, #G.jokers.cards do
                    if next(SMODS.find_card("j_dusk")) and card.ability.extra.gain == 0.05 then
                        card.ability.extra.gain = card.ability.extra.gain * 2
                        break
                    elseif not next(SMODS.find_card("j_dusk")) and card.ability.extra.gain ~= 0.05 then
                        card.ability.extra.gain = card.ability.extra.gain / 2
    ...
    calculate = function(self, card, context)
        ...
        if next(SMODS.find_mod("partner")) then
            if G.GAME.selected_partner == "pnr_fur_lumepartner" then
                if card.ability.partnersynergizetrigger == "False" or card.ability.extra.xmult == 2 then
                    card.ability.partnersynergizetrigger = "True"
                    card.ability.extra.xmult = card.ability.extra.xmult * 2
                end
            end
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        if card.debuff then return nil end
        for i = 1, #G.jokers.cards do
            if next(SMODS.find_card("j_ice_cream")) and card.ability.extra.xmult == 1.5 then
                card.ability.extra.xmult = card.ability.extra.xmult * 1.5
                break
            elseif not next(SMODS.find_card("j_ice_cream")) and card.ability.extra.xmult ~= 1.5 then
                card.ability.extra.xmult = card.ability.extra.xmult / 1.5
                break
            end
        end
        ...
        calculate = function(self, card, context)
            ...
            if next(SMODS.find_mod("partner")) then
                if G.GAME.selected_partner == "pnr_fur_sephirapawspartner" then
                    if card.ability.partnersynergizetrigger == "False" or card.ability.extra.gain == 0.1 then
                        card.ability.partnersynergizetrigger = "True"
                        card.ability.extra.gain = card.ability.extra.gain * 2
                    end
                end
