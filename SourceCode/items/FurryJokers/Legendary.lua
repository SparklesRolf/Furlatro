...
SMODS.Joker {
    ...
    in_pool = function(self, args)
        local corruptsparkles = 0

        for i = 1, #G.jokers.cards do
            if G.jokers.cards[i].config.center.key == "j_fur_talismancorruptsparkles" or G.jokers.cards[i].config.center.key == "j_fur_corruptsparkles" then
                corruptsparkles = corruptsparkles + 1
            end
        end

        if corruptsparkles >= 1 then
            return false
        else
            return true
        end
