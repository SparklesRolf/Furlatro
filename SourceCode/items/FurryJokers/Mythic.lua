...
SMODS.Joker { 
    ...
    in_pool = function(self, args)
        if next(SMODS.find_card("j_ring_master")) then
            return true
        elseif (next(SMODS.find_card("j_fur_corruptcuriousangel")) or next(SMODS.find_card("j_fur_talismancorruptcuriousangel"))) then
            return false
        else
            return true
        end
    ...
    SMODS.Joker {
        ...
        calculate = function(self, card, context)
            ...
            if context.using_consumeable then
                if G.GAME.selected_partner ~= "pnr_fur_dannypartner" and (context.consumeable.config.center.key == 'c_fur_fallingstar' or context.consumeable.config.center.key == 'c_six_star_q') then
	                for i = 1, #G.hand.highlighted do
                        SMODS.debuff_card(G.hand.highlighted[i], false, "fur_danny")
                    end
                elseif G.GAME.selected_partner ~= "pnr_fur_dannypartner" and (context.consumeable.config.center.key == 'c_sun' or context.consumeable.config.center.key == 'c_moon' or context.consumeable.config.center.key == 'c_world' or context.consumeable.config.center.key == 'c_star' or context.consumeable.config.center.key == 'c_lovers' or context.consumeable.config.center.key == 'c_six_moon_q') then
                    for i = 1, #G.hand.highlighted do
            ...
            if context.selling_self then
                ...
                for i = 1, #G.jokers.cards do
                    if G.jokers.cards[i].config.center.key == "j_fur_talismandanny" then
                        dannycards = dannycards + 1
                    end
                end

                if dannycards == 1 then
                    if G.GAME.selected_partner ~= "pnr_fur_dannypartner" then
	    	            if G.deck and card.added_to_deck then
	    	        	    for i, v in pairs(G.deck.cards) do
	    	        	    	if not (v:is_suit("fur_stars") or v:is_suit('six_Stars')) then
	    	        	    		SMODS.debuff_card(v, false, "fur_danny")
	    	        	    	end
	    	        	    end
	    	            end
        ...
        add_to_deck = function(self, card, from_debuff)
            if G.GAME.selected_partner ~= "pnr_fur_dannypartner" then
	    	    if G.deck and card.added_to_deck then
	    		    for i, v in pairs(G.deck.cards) do
	    		    	if not (v:is_suit("fur_stars") or v:is_suit('six_Stars')) then
	    		    		SMODS.debuff_card(v, true, "fur_danny")
	    		    	end
	    		    end
	    	    end
	    	    if G.hand and card.added_to_deck then
        ...
        remove_to_deck = function(self, card, from_debuff)
            local dannycards = 0
            for i = 1, #G.jokers.cards do
                if G.jokers.cards[i].label == "j_fur_talismandanny" then
                    dannycards = dannycards + 1
                end
            end
            
            if G.GAME.selected_partner ~= "pnr_fur_dannypartner" and dannycards == 1 then
	    	    if G.deck and card.added_to_deck then
	    		    for i, v in pairs(G.deck.cards) do
	    		    	if not (v:is_suit("fur_stars") or v:is_suit('six_Stars')) then
	    		    		SMODS.debuff_card(v, false, "fur_danny")
        ...
        in_pool = function(self, args)
            if next(SMODS.find_card("j_ring_master")) then
                return true
            elseif next(SMODS.find_card("j_fur_talismancorruptdanny")) then
                return false
            else
                return true
            end
...
if next(SMODS.find_mod("Multiplayer")) then
    SMODS.Joker { 
        ...
        loc_vars = function(self, info_queue, card)
            ...
            if config.joker_lines then
                return { vars = {numerator, denominator, skips_lines.normal[math.random(#skips_lines.normal)] }}
            else
...
local negativenan = -1000000
local infinity = 1.8e308
local ignaril_lines = { -- ignaril Assacire
    normal = {
        '"We got to have more money"',
        '"♪Welcome to my shop♪"',
        '"123, tap your feet, such a classic melody"',
        '"What is one, against infinity?"',
        '"FEED ME! I HUNGER FOR KROMER!"',
        '"Do I look like I know what I\'m DUWANG?"',
        '"SHOOT THE SPOTLIGHT MY WAY!"',
        '"Money? A safety, obviously!"',
    },
    toggle = {
        '',
    },
...
SMODS.Joker {
    ...
    calculate = function(self, card, context)
        if card.debuff then return nil end
        if G.GAME.selected_partner == "pnr_fur_ignarilassacirepartner" then
            if not card.ability.partnersynergizetrigger then
                card.ability.partnersynergizetrigger = true
        ...
        if context.retrigger_joker_check and (context.other_card.config.center.rarity == "fur_rarityfurrynonmythic" or context.other_card.config.center.rarity == "fur_rarityfurrymythic" or context.other_card.config.center.rarity == "fur_rarityfurrycorrupt") and not context.retrigger_joker and not context.ending_shop then
            return {
    ...
    loc_vars = function(self, info_queue, card)
        if config.joker_lines then
            return { vars = { card.ability.extra.retriggers, ignaril_lines.normal[math.random(#ignaril_lines.normal)]}}
        else
            return { vars = { card.ability.extra.retriggers, ignaril_lines.toggle[math.random(#ignaril_lines.toggle)]}}
        end
    end,
...
SMODS.Joker {
    ...
    in_pool = function(self, args)
        if next(SMODS.find_card("j_ring_master")) then
            return true
        elseif (next(SMODS.find_card("j_fur_corruptiridia")) or next(SMODS.find_card("j_fur_talismancorruptiridia"))) then
            return false
        else
            return true
        end
...
if Talisman then
    SMODS.Joker {
        ...
        config = { partnersynergizetrigger = false, extra = { xmoney = 0.1, moneygain = 0.1 }},
        atlas = 'joseeeeslab',
        pos = { x = 1, y = 0 },
        soul_pos = { x = 0, y = 0 },
        rarity = "fur_rarityfurrymythic",
        ...
        calculate = function(self, card, context)
            ...
            if context.joker_main then
                return {
                    ...
                    chip_message = { message = "^"..card.ability.extra.echips.." "..localize('k_mult'), colour = G.C.DARK_EDITION },
                    emult = card.ability.extra.emult,
                    sound = 'fur_emult',
                    colour = G.C.DARK_EDITION
                }
    ...
    SMODS.Joker {
        ...
        calculate = function(self, card, context)
            ...
            if context.setting_blind then
                ...
                if not context.blueprint then
                    card_eval_status_text(card, 'extra', nil, nil, nil, {message = localize("loyalty_active", "v_dictionary"), colour = G.C.FILTER})
                end
            end
        ...
        in_pool = function(self, args)
            if next(SMODS.find_card("j_ring_master")) then
                return true
            elseif next(SMODS.find_card("j_fur_talismancorruptsephirapaws")) then
                return false
            else
                return true
            end
...
if Talisman then
    SMODS.Joker {
        ...
        config = { multstate = false, extra = { money = 5, emult = 2 }},
        ...
        calculate = function(self, card, context)
            if card.debuff then return nil end
            if context.before then
                if G.jokers.cards[1] == card and G.jokers.cards[#G.jokers.cards] ~= card then
                    ease_dollars(card.ability.extra.money)

                    return {
                        extra = { card = card, message = "+$"..card.ability.extra.money, colour = G.C.MONEY },
                    }
                elseif G.jokers.cards[#G.jokers.cards] == card and G.jokers.cards[1] ~= card then
                    if G.GAME.selected_partner == "pnr_fur_sourstone3partner" then
                        card.ability.multstate = true
                    elseif to_number(G.GAME.dollars) > (G.GAME.bankrupt_at + (card.ability.extra.money - 1)) then
                        card.ability.multstate = true
                        ease_dollars(-card.ability.extra.money, true)

                        return {
                            extra = { card = card, message = "-$"..card.ability.extra.money, no_retrigger = true, colour = G.C.RED },
                        }
                    else
                        ...
                        return {
                            extra = { card = card, message = localize("k_nope_ex"), no_retrigger = true, colour = G.C.FILTER },
                        }
                    end
                end
            end
            ...
            if context.joker_main then
                if (G.jokers.cards[#G.jokers.cards] == card and card.ability.multstate) or (G.jokers.cards[1] == card and G.jokers.cards[#G.jokers.cards] == card) then
                    if not context.blueprint then
                        card.ability.multstate = false
                    end
