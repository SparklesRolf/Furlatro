    ...
    SMODS.Joker {
        ...
        calculate = function(self, card, context)
            ...
            if context.using_consumeable then
                if G.GAME.selected_partner ~= "pnr_fur_dannypartner" and (context.consumeable.config.center.key == 'c_fur_fallingstar' or context.consumeable.config.center.key == 'star_q') then
	                for i = 1, #G.hand.highlighted do
                        SMODS.debuff_card(G.hand.highlighted[i], false, "fur_danny")
                    end
                elseif G.GAME.selected_partner ~= "pnr_fur_dannypartner" and (context.consumeable.config.center.key == 'c_sun' or context.consumeable.config.center.key == 'c_moon' or context.consumeable.config.center.key == 'c_world' or context.consumeable.config.center.key == 'c_star' or context.consumeable.config.center.key == 'moon_q') then
                    for i = 1, #G.hand.highlighted do
