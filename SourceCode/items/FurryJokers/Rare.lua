    ...
    SMODS.Joker {
        ...
        calculate = function(self, card, context)
            ...
            if context.selling_self then
                ...
                for i = 1, #G.jokers.cards do
                    if G.jokers.cards[i].config.center.key == "j_fur_danny" then
        ...
        in_pool = function(self, args)
            if next(SMODS.find_card("j_ring_master")) then
                return true
            elseif next(SMODS.find_card("j_fur_corruptdanny")) then
                return false
            else
                return true
            end
    ...
    calculate = function(self, card, context)
        ...
        if context.setting_blind then
            ...
            return {
                extra = {
                    card = card,
                    message = localize("fur_kalikstink"),
...
SMODS.Joker {
    key = 'maltnoodlez',
    config = { inround = false, debuffedjoker = nil },
    ...
    calculate = function(self, card, context)
        ...
        if context.setting_blind then
            ...
            if G.GAME.selected_partner == "pnr_fur_maltnoodlezpartner" then
                ...
                if not context.blueprint then
                    ...
                    if _card then
                        debuffedjoker = _card
                        card.ability.debuffedjoker = _card.label
        ...
        if ((G.GAME.blind or G.GAME.blind.boss) and context.end_of_round) or context.selling_self then -- From VanillaRemade
            ...
            if not context.blueprint then
                ...
                for i = 1, #G.jokers.cards do
                    if G.jokers.cards[i].debuff and G.jokers.cards[i].label == card.ability.debuffedjoker then
                        table.insert(jokers, G.jokers.cards[i])
                    end
                end
                local _card = jokers[math.random(#jokers)]
...
SMODS.Joker {
    ...
    add_to_deck = function(self, card, from_debuff)
        ...
        G.E_MANAGER:add_event(Event({
            ...
            delay = 0.0,
            func = (function()
                if not next(SMODS.find_card("c_fur_nemzsbow")) then
                    local card = create_card('Tarot',G.consumeables, nil, nil, nil, nil, "c_fur_nemzsbow", 'fur_nemzata')
                    card:add_to_deck()
...
SMODS.Joker {
    key = "potmario",
    config = { partnersynergizetrigger = false, extra = { chips = 0, xmult = 1, chipsgain = 15, xmultgain = 0.5, money = 10 }},
    atlas = 'potmarioslab',
    pos = { x = 1, y = 0 },
    soul_pos = { x = 0, y = 0 },
    rarity = "fur_rarityfurrynonmythic",
    ...
    calculate = function(self, card, context)
        if card.debuff then return nil end
        if G.GAME.selected_partner == "pnr_fur_potmariopartner" then
            if not card.ability.partnersynergizetrigger then
                card.ability.partnersynergizetrigger = true
                card.ability.extra.chipsgain = card.ability.extra.chipsgain * 2
                card.ability.extra.xmultgain = card.ability.extra.xmultgain * 2
                card.ability.extra.money = card.ability.extra.money * 2
            end
        end
        ...
        if context.round_eval and G.GAME.blind.boss and (G.jokers.cards[1] == card and G.jokers.cards[#G.jokers.cards] == card) then
            G.E_MANAGER:add_event(Event({
                func = (function()
                    if G.GAME.selected_partner == "pnr_fur_potmariopartner" then
    ...
    loc_vars = function(self, info_queue, card)
        ...
        if config.joker_lines then
            return { vars = { card.ability.extra.chips + (card.ability.extra.chipsgain * furrycards), card.ability.extra.xmult + (card.ability.extra.xmultgain * furrycards), card.ability.extra.chipsgain, card.ability.extra.xmultgain, card.ability.extra.money, localize("k_ante"), potmario_lines.normal[math.random(#potmario_lines.normal)]}}
        else
            return { vars = { card.ability.extra.chips + (card.ability.extra.chipsgain * furrycards), card.ability.extra.xmult + (card.ability.extra.xmultgain * furrycards), card.ability.extra.chipsgain, card.ability.extra.xmultgain, card.ability.extra.money, localize("k_ante"), potmario_lines.toggle[math.random(#potmario_lines.toggle)]}}
        end
    end,
...
if not Talisman then
    SMODS.Joker {
        key = 'sourstone3',
        config = { multstate = false, extra = { money = 5, xmult = 3 }},
        ...
        calculate = function(self, card, context)
            ...
            if context.before then
                ...
                elseif G.jokers.cards[#G.jokers.cards] == card and G.jokers.cards[1] ~= card then
                    ...
                    elseif G.GAME.dollars > (G.GAME.bankrupt_at + (card.ability.extra.money - 1)) then
                        card.ability.multstate = true
                        if G.GAME.selected_partner ~= "pnr_fur_sourstone3partner" then
