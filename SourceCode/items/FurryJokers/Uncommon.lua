...
local gale_lines = { -- Gale
    normal = {
        '*Assorted rabbit noises*',
        '"So I heard you like stone cards..."',
...
SMODS.Joker {
    key = 'gale',
    config = { partnersynergizetrigger = false, extra = { chips = 0, mult = 0, chipsgain = 100, multgain = 10, xmult = 2 }},
    atlas = 'galeslab',
    pos = { x = 1, y = 0 },
    soul_pos = { x = 0, y = 0 },
    ...
    calculate = function(self, card, context)
        ...
        if context.before and not context.blueprint then
            ...
            for _, card in ipairs(context.full_hand) do
                if card.debuff or (SMODS.has_enhancement(card, 'm_stone') and card.config.center.key ~= "m_fur_snowflakecard") and not context.blueprint then
                    card:set_ability('m_fur_snowflakecard', nil, true)
                    convertedcards = true
                ...
                if card.debuff then
                    SMODS.debuff_card(card, false, "fur_danny")
                end
            end
    ...
    loc_vars = function(self, info_queue, card)
        ...
        info_queue[#info_queue + 1] = G.P_CENTERS.m_fur_snowflakecard
        local snowflakecardcount = 0
        ...
        if config.joker_lines then
            return { vars = { card.ability.extra.chipsgain, card.ability.extra.multgain, card.ability.extra.chips + (card.ability.extra.chipsgain * snowflakecardcount), card.ability.extra.mult + (card.ability.extra.multgain * snowflakecardcount), card.ability.extra.xmult, gale_lines.normal[math.random(#gale_lines.normal)]}}     
        else
            return { vars = { card.ability.extra.chipsgain, card.ability.extra.multgain, card.ability.extra.chips + (card.ability.extra.chipsgain * snowflakecardcount), card.ability.extra.mult + (card.ability.extra.multgain * snowflakecardcount), card.ability.extra.xmult, gale_lines.toggle[math.random(#gale_lines.toggle)]}}
        end
    end,
...
SMODS.Joker {
    key = 'silver',
    config = { partnersynergizetrigger = false, extra = { money = 4 }},
    atlas = 'silverslab',
    pos = { x = 1, y = 0 },
    soul_pos = { x = 0, y = 0 },
    rarity = 'fur_rarityfurrynonmythic',
