...
local star_suit = SMODS.Suit {
    ...
    pos = { y = 0 },
    ui_pos = { x = 0, y = 0 },
    hc_colour = HEX('9971DF'),
    lc_colour = HEX('FD65FF'),
    in_pool = allow_suits
}

if config.star_suit then -- Flush Buff if Stars
    SMODS.PokerHand:take_ownership('Flush', { -- Buffing Flushes if stars are enabled (More cards = less odds to pull) *From VanillaRemade*
        key = "Flush",
        mult = 6,
        chips = 50,
        l_mult = 3,
...
SMODS.Blind { -- The Paw
    ...
    recalc_debuff = function(self, card, from_blind)
		if (card.area == G.jokers) and not G.GAME.blind.disabled and (card.config.center.rarity == 'fur_rarityfurrynonmythic' or card.config.center.rarity == 'fur_rarityfurrymythic' or card.config.center.rarity == 'fur_rarityfurrycorrupt') then
			return true
		end
		return false
	end,
