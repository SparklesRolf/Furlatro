    ...
    apply = function(self, tag, context)
		if context.type == "store_joker_create" then
			...
			for k, v in ipairs(G.jokers.cards) do
				if (v.config.center.rarity == 'fur_rarityfurrynonmythic' or v.config.center.rarity == 'fur_rarityfurrymythic') and not rares_in_posession[v.config.center.key] then
					rares_in_posession[1] = rares_in_posession[1] + 1
					rares_in_posession[v.config.center.key] = true
				end
			end
			local card
			if (#G.P_JOKER_RARITY_POOLS.fur_rarityfurrynonmythic or #G.P_JOKER_RARITY_POOLS.fur_rarityfurrynmythic) > rares_in_posession[1] then
