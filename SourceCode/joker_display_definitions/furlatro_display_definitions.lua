...
jd_def["j_fur_spark"] = { -- SparkTheBird
    ...
    calc_function = function(card)
        ...
        card.joker_display_values.xmult = card.ability.extra.xmult ^ count
        card.joker_display_values.xmult2 = card.ability.extra.xmult2 ^ count2
...
jd_def["j_fur_skips"] = { -- DelusionalSkips
    ...
    calc_function = function(card)
        if G.GAME.blind.boss then
            if G.GAME.probabilities.normal > 2 then
                card.joker_display_values.odds = localize { type = 'variable', key = "jdis_odds", vars = { 2, card.ability.extra.odds } }
            else
                card.joker_display_values.odds = localize { type = 'variable', key = "jdis_odds", vars = { (G.GAME and G.GAME.probabilities.normal or 1), card.ability.extra.odds } }
            end
        else
